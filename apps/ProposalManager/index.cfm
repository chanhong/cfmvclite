		...
		<tr id="asset#q.id#">
			...
			<td>
				<cfif assetType is "Bio">
					<a href="cfc/asset.cfc?method=downloadBioPDFfile&id=#id#" target="_blank">
						[download Bio PDF]
					</a>
				<cfelse>
					<cfif q.filename is not "">
						<a href="cfc/asset.cfc?method=downloadfile&id=#id#">
							[download]
						</a>
					</cfif>
				</cfif>
			...
			<td>
				<a href="admin/asset.cfm?id=#q.id#">
					[Edit]
				</a>
			</td>
			<td>
				<a href="javascript:deleteRecord(#q.id#,'#jsStringFormat(q.title)#')">
					[Del]
				</a>
			</td>
		</tr>
...
<cfoutput>
	<input type="button" value="Download List" 
	       onclick="location.href='cfc/asset.cfc?method=downloadspreadsheet&searchterm=#urlencodedformat(form.searchterm)#'">
</cfoutput>
