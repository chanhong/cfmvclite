...
<cffunction name="captureContents">
		<cfargument name="fspec" required="true" type="string">
		<cfargument name="args" required="false" type="struct">
...
<cfparam
	...
	<cfset var $ = { }>
	<cfset structAppend( local, args )>
	<cfif ( isNull( response ) )>
	<cfset	response = ''>
	<cfsavecontent variable="response">
			<cfinclude template='#fspec#'>
	</cfsavecontent>
	</cfif>
...
<cfparam
	...
	<cfset var $ = { }>
	<cfset structAppend( local, args )>
	<cfreturn captureContents(fspec, args)>			
</cffunction>

<cffunction name="renderLayout">
		<cfargument name="fspec" required="true" type="string">
		<cfargument name="args" required="false" type="struct">
